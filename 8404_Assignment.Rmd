...
i
ridge_cv_fit$cvm[i]
coef(ridge_cv_fit,s=lambda_min)
```
```{r}
X2=test_data[,1:10]
...
A
b=diabetes$dis
ridge_fit1=glmnet(A,b,alpha = 0,standardize = TRUE,lambda = grid)
bet3=coef(ridge_fit1)
dim(bet3)
grid[1]
bet3[,1]
plot(ridge_fit1,xvar = "lambda",col=1:10,label = TRUE)
ridge_cv_fit1=cv.glmnet(A,b,alpha=0,standardize=TRUE,lambda = grid)
plot(ridge_cv_fit1)
```
...
lambda_min1
i=which(ridge_cv_fit1$lambda==ridge_cv_fit1$lambda.min)
i
ridge_cv_fit$cvm[i]
coef(ridge_cv_fit1,s=lambda_min)
#summary(ridge)
#ridge_fit1$lambda_min
#bet4=coef(ridge_fit1,s=ridge_fit1$lambda.min)

#bet4
```
#regression coefficients are age,sex,tc are tending towards zero
