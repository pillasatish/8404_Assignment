...
```{r}
set.seed(12)
train_data=diabetes[1:350,]
test_data=diabetes[351:442,]

```
#fitting the multiple linear regression model
```{r}
model1=lm(dis~.,data=train_data)
summary(model1)
...
```{r}
library(leaps)
bss_fit=regsubsets(dis~.,data=train_data,method="exhaustive",nvmax=11)
...
test_data1=data.frame(train_data$sex,train_data$bmi,train_data$map,train_data$tc,train_data$hdl,train_data$ltg,train_data$dis)
model2=lm(train_data.dis~.,data=train_data1)
pred1=predict(model2,newdata = test_data1)
pred1
MSE2=mean((test_data1$train_data.dis-pred1)^2)
MSE2
```
#Rigid regression
```{r}
